<?php
//creating cookie to store selected category, start------------------------------------------------------------------------------------------------
    if (isset($_POST['cat'])){
        //setting the cookies name and value
        $cookie_name = "Category";
        $cookie_value = $_POST['cat'];

        //setting category cookies time expiry to 3 days
        setcookie($cookie_name, $cookie_value,time() + (86400 * 3), "/");
    
        //setting category cookie to the category the user selects 
        $_COOKIE['Category'] = $_POST['cat'];
    }
//end----------------------------------------------------------------------------------------------------------------------------------------------

//selecting a specific set of words depending on the category selected by the user, start----------------------------------------------------------
    //if the user selects "Movies", then this set of words will be selected
    if($_COOKIE['Category'] == "Movies"){
        
        $movies = array("Batman", "Superman", "Avengers", "StarWars", "UP", "WonderWoman", "Rocky", "Ironman");
    }
    //if the user selects "Countries", then this set of words will be selected
    else if($_COOKIE['Category'] == "Countries"){
        
        $movies = array("Australia", "Brazil", "Russia", "China", "Germany", "France", "Italy", "Austria", "Mexico");
    }
    //if the user selects "Names", then this set of words will be selected
    else{
        
        $movies = array("Daniel", "Rodney", "Jarred", "Brent", "Justine", "Andrew", "Paul", "Jackson", "Brad", "Kemp", "Jack");    
    }
//end----------------------------------------------------------------------------------------------------------------------------------------
...
//if the user clicks "New Word" then a random word will be selected from the category, then underscores will be generated according to the number of letters in the word, then cookies will be created for the Wrong Letters, start----------------------------         
    ...
    if($_POST['Word']){  
        ...
        //shuffling the words in the selected category
        shuffle($movies);
            
        //selecting and saving the first word from the selected category
        $_POST['Answer'] = $movies[0]; 
            
        //counting how many letters are in the word, then saving it
        $underscore = strlen($_POST['Answer']);
...
<html>
    ...
    <body>
        ...
        <!-------------------------------------buttons to select category, start------------------------------------>
        <div id="cate">
            <h2>The next words category!</h2>
            <input type="submit" name="cat" value="Movies">
            <input type="submit" name="cat" value="Countries">
            <input type="submit" name="cat" value="Names"> <br>
        </div>
        ...
        //ending the game if there has been 6 wrong guesses, start----------------------------------------------------------------------------------------    
            ...
            if($Wrong == 6){
                ...
                echo "<p2><br>Better luck next time " . $_COOKIE['PlayerName'] . ", the word was " . $_POST['Answer'] . "</p2>";
        ...
        //if there's no "_" in $_array, then the user wins, start---------------------------------------------------------------------------------
            ...
            //if there isn't any "_", then the following code runs
                ...
                echo "<p2><br> You win " . $_COOKIE['PlayerName'] . "!</p2>"; 
            }
        //end-------------------------------------------------------------------------------------------------------------------------------------   
        ...
        //end-------------------------------------------------------------------------------------------------------------------------------------
        ?>         
            <br><br><br><input type="submit" name="Word" value="New Word"><br><br>        
        <?php    
        ...
        //end-------------------------------------------------------------------------------------------------------------------------------------        
            ...
            echo "<br>";
            echo "The category you have selected is " . $_COOKIE['Category'];
        ?>
        </center>
        </form>     
    </body>
